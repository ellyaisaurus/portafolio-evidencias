document.addEventListener('DOMContentLoaded', () => {
    ...
    const pokemonListElement = document.getElementById('pokemon-list'); // UL Element (Requirement #4)
    ...
    const fetchData = async () => {
        ...
        if(pokemonListElement) pokemonListElement.innerHTML = ''; // Limpiar UL
        ...
        try {
            ...
            if (currentState.pokemonList.length === 0) {
                ...
                // --- 4. Verificación del Contenedor UL ---
                if (pokemonListElement) {
                    
                    // 5. Iterar sobre los datos (ForEach)
                    currentState.pokemonList.forEach(poke => {
                        
                        // 6. Crear elemento LI
                        const li = document.createElement('li');
                        
                        // 7. Agregar clase Bootstrap 'list-group-item'
                        li.classList.add('list-group-item');
                        
                        // Agregar contenido (Nombre + extras para UI)
                        // NOTA: El requisito dice "establece su contenido como el nombre".
                        // Lo hacemos, pero agregamos HTML para que se vea bien en la UI, manteniendo el nombre visible.
                        li.innerHTML = `
                        ...
                        // Evento click extra para mejorar UX en la lista
